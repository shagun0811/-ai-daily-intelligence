...
function renderGallery(report) {
  ...
  const player = files.mp4
    ? `<video id="briefing-video" class="gallery-video" controls playsinline preload="metadata" src="./${escapeHtml(files.mp4)}" title="Briefing video for ${escapeHtml(report.report_date)}"></video>`
    : "";
...
function renderDownloads(report) {
  ...
  [watchMasthead, watchHero].forEach((el) => {
    if (!el) return;
    if (mp4) {
      const filename = mp4.split("/").pop() || "briefing.mp4";
      el.href = `./${mp4}`;
      el.setAttribute("download", filename);
      el.setAttribute("aria-label", `Watch or download this day’s briefing video`);
      el.onclick = (event) => {
