...
function bindRssCopy() {
  ...
  if (!rss || rss.dataset.bound === "1") return;
  rss.dataset.bound = "1";
  rss.addEventListener("click", async (event) => {
    if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;
    event.preventDefault();
    const url = new URL(rss.getAttribute("href") || "feed.xml", window.location.href).href;
    try {
      await navigator.clipboard.writeText(url);
      const label = rss.dataset.label || rss.textContent || "RSS";
      rss.dataset.label = label;
      rss.textContent = "Copied";
      window.setTimeout(() => {
        rss.textContent = rss.dataset.label || "RSS";
      }, 1400);
    } catch (error) {
...
function layoutBriefing(briefing, reportDate) {
  ...
  const gossip = [];
  hero.forEach((item) => {
    if (isGossipForHero(item) && !isSubstanceStory(item)) gossip.push(item);
    else substance.push(item);
  });
  const ordered = substance.length ? [...substance, ...gossip] : [...gossip];
  const shown = ordered.slice(0, 5);
  const heroKeys = new Set(shown.map((item) => titleKey(item.title)));
  const more = [...demoted, ...ordered.slice(5)];
  const research = [];
...
function storyBlob(item) {
  return `${item.title || ""} ${item.summary || ""} ${item.why_it_matters || ""} ${item.body || ""} ${item.source_url || ""}`
    .toLowerCase()
    .replace(/-/g, " ");
}

function isGossipForHero(item) {
  const text = storyBlob(item);
  return ["decoder", "podcast", "org chart", "executive exodus", "consolidate power", "who reports to", "management shuffle"].some((term) => text.includes(term));
}

function isSubstanceStory(item) {
  const text = storyBlob(item);
...
function renderGallery(report) {
  ...
  if (files.video && !files.mp4) images.push([files.video, `Briefing animation for ${report.report_date}`]);
  const cards = files.cards || [];
  const filename = files.mp4 ? (files.mp4.split("/").pop() || "briefing.mp4") : "";
  const poster = files.infographic ? ` poster="./${escapeHtml(files.infographic)}"` : "";
  const player = files.mp4
    ? `<div class="video-block">
        <video id="briefing-video" class="gallery-video" controls playsinline preload="metadata"${poster} src="./${escapeHtml(files.mp4)}" title="Briefing video for ${escapeHtml(report.report_date)}"></video>
        <p class="video-actions"><a class="save-link" href="./${escapeHtml(files.mp4)}" download="${escapeHtml(filename)}">Download MP4</a></p>
      </div>`
    : "";
...
function renderDownloads(report) {
  ...
  const watchMasthead = document.getElementById("watch-video");
  const watchHero = document.getElementById("watch-video-hero");
  ...
  [watchMasthead, watchHero].forEach((el) => {
    if (!el) return;
    if (mp4) {
      el.href = `#briefing-video`;
      el.removeAttribute("download");
